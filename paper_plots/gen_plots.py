...
title = 'Early train'
train_vector_field = pd.read_csv('/home/casa/projects/saved_runs/fake-birdview-gan/0/runs/env_info/1/1.csv')
episode_len = 12282
vector_field = train_vector_field[(train_vector_field['n_epoch'] < 10) & (
    train_vector_field['num_timesteps'] < 8 * episode_len)]
...
vector_field = train_vector_field[(train_vector_field['num_timesteps'] >= 5 * episode_len) & (
    train_vector_field['num_timesteps'] < 13 * episode_len)]
...
vector_field = train_vector_field[(train_vector_field['num_timesteps'] >= 20 * episode_len) & (
    train_vector_field['num_timesteps'] < 28 * episode_len)]
img_name = 'late_train'
title = 'Late train'
generate_vector_field(vector_field, title, ax_count)
