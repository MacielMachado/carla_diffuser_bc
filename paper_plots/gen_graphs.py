...
# all_results_dir = Path('paper_plots/results2')
# results_subdirs = ['bc-gail-gan', 'bc-gail']
# results_labels = {'bc-gail-gan': 'hGAIL', 'bc-gail': 'BC GAIL', 'bc-gan': 'BC', 'rgb': 'GAIL from cam', 'ppo': 'PPO', 'gail': 'GAIL'}  # 
# results_types = ['eval_episodes', 'eval_meters', 'rollout_episodes', 'rollout_meters', 'rollout_steps']
# columns_names = ['eval/completed_n_episodes', 'eval/route_completed_in_m', 'rollout/completed_n_episodes', 'rollout/route_completed_in_m', 'rollout/step']
# columns_labels = {'eval/completed_n_episodes': 'infractions', 'eval/route_completed_in_m': 'route completed in m', 'rollout/completed_n_episodes': 'infractions', 'rollout/route_completed_in_m': 'route completed in m', 'rollout/step': 'step'}
all_results_dir = Path('paper_plots/results2')
results_subdirs = ['bc-gail-gan', 'bc-gail']
...
for result_type, column_name in zip(results_types, columns_names):
    for results_subdir in results_subdirs:
        ...
        if results_subdir == 'bc-gan' and not result_type in ['eval_episodes', 'eval_route_completed']:
            continue
        if results_subdir == 'rgb' and not result_type in ['rollout_episodes', 'eval_route_completed']:
            continue
        if results_subdir == 'no-plot-bc-gail-gan' and not result_type in ['eval_route_completed']:
            continue
        if results_subdir == 'no-plot-rgb' and not result_type in ['rollout_episodes']:
            continue
        results_dfs = []
    ...
    if result_type == 'eval_route_completed':
