...
from rl_train import get_obs_configs, get_env_wrapper_configs, env_eval_maker, env_maker
...
if __name__ == '__main__':
    ...
    env_eval = SubprocVecEnv([lambda env_id=env_id, config=config: env_maker(env_id, config, obs_configs, env_wrapper_configs, rendering=generate_rgb) for env_id, config in enumerate(multi_env_eval_configs)])
    ...
    _eval_step = int(1e5)
    n_steps_total = 12288
    wb_run_path = 'gustavokcouto/gail-carla2/ol8xukb8'
    api = wandb.Api()
    ...
    for ckpt_file in all_ckpts:
        ...
        if (num_timesteps % _eval_step) < n_steps_total:
            ...
            avg_ep_stat, avg_route_completion, ep_events = WandbCallback.evaluate_policy(env_eval, policy, eval_video_path)
